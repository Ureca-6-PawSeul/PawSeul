...
const useCartStore = create<CartStore>((set) => ({
    ...
    set((state) => {
      ...
      return {
        selectedItems: isSelected
          ? state.selectedItems.filter((item) => item.productId !== product.productId)
  ...
  toggleSelectAll: () =>
    set((state) => ({
      selectedItems:
        state.selectedItems.length === state.cartItems.length ? [] : [...state.cartItems],
    })),
  ...
  deleteSelectedItems: () =>
    set((state) => ({
      cartItems: state.cartItems.filter(
        (item) => !state.selectedItems.some((selected) => selected.productId === item.productId)
      ),
      selectedItemIds: [],
    })),
